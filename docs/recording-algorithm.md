...
| `SILENCE_DISCARD_FRAMES` | 25 | 2.00 | silence ≥ this: flush buffer, discard silence, emit `NOTE gap` |
...
(or ≥ 25 frames measured at commit). Rendered as a `NOTE gap` block. The
audio of a gap exists in `audio.webm`; it is simply not covered by any cue.
...
- `run.label == SILENCE`:
  1. If `run.length ≥ SILENCE_DISCARD_FRAMES`: if the buffer is non-empty,
     ...
     outbox. Normally the buffer was already flushed when the open run
     crossed 25 frames. Do not append anything.
...
5. PCM = ring.slice(buffer.startFrame · 1280, buffer.endFrame · 1280)
   # one contiguous slice: buffer items are contiguous (§3), so embedded
   # glue silences are included — natural audio for the ASR
...
| 2 | Idle | SILENCE_COMMITTED ≥ 25 | emit NOTE gap → Idle |
...
| 10 | Buf(S) | SILENCE_COMMITTED ≥ 25 | safety-net `flush(.longSilence)`, then emit NOTE gap → Idle (rare: only when the run closed before its latched ≥ 25 progress event fired, e.g. a reopen-merge jump followed immediately by a label change; normally row 12 already emptied the buffer and this event arrives in Idle, row 2) |
| 11 | Buf(S) | SPEECH_OPEN stable, speaker S | MAX check → Buf(S) |
| 12 | Buf(S) | SILENCE_OPEN ≥ 25 (latched) | flush(.longSilence) → Idle |
...
(f55–89), 0.24 s gap (f90–92), A speaks 7.44–9.60 (f93–119), then long
silence.
...
| f145: silence reaches 25 frames | **flush(.longSilence)** → **cue 3: Speaker A, 00:07.440 → 00:09.600**; silence keeps running |
| silence run eventually commits / stop | ≥ 25 frames ⇒ `NOTE gap start=00:09.600 end=…` |
...
7.04–13.04 (f88–162), then long silence.
...
| f188 (silence = 25) | **flush(.longSilence)** → **cue 5 (`NOTE continuation`): Speaker A, 00:07.040 → 00:13.040** |
...
### 12.3 Pause ≥ 2 s, same speaker resumes

Audio: A speaks 0.00–3.04 (f0–37), 2.80 s silence (f38–72), A speaks
5.84–8.00 (f73–99), user presses Stop at 9.00 s (`realSamples = 144 000`,
`realFrameEnd = 113`).

```
f:      0.......37|38...............72|73........99|100...112|(stop)
label:  AAAAAAAAAA SSSSSSSSSSSSSSSSSSS AAAAAAAAAAAA SSSSSSSSS
            run1          run2             run3        run4
```
...
| f63 (silence = 25) | **flush(.longSilence)** → **cue 6: Speaker A, 00:00.000 → 00:03.040**; buffer → Idle; silence keeps running |
| f75 (A stable) | run2 sil[38,73) commits: 35 frames ≥ 25 ⇒ **`NOTE gap start=00:03.040 end=00:05.840`** (nothing to glue: Idle) |
| f101 | run3 A[73,100) commits → buffer(A) |
| stop | pad zeros → frames ≥ 113 forced SILENCE; run4 sil[100,113) = 13 frames < 25 ⇒ no gap NOTE; **flush(.stop)** → **cue 7: Speaker A, 00:05.840 → 00:08.000**; `NOTE session 1 end … offset=00:00:09.000` |
...
NOTE gap start=00:03.040 end=00:05.840

7
00:05.840 --> 00:08.000
<v Speaker 1>…

NOTE session 1 end=… offset=00:00:09.000
```

`audio.webm` contains all 9.00 s including both silences; every cue timestamp
seeks correctly into it.
...
3. Union of (cues ∪ gap NOTEs ∪ sub-2 s uncovered gaps ∪ leading/trailing
   silence) = the whole note timeline; no sample is double-covered by cues.
