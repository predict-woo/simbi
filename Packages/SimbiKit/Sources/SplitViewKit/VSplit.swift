...
public struct VSplit<P: View, D: SplitDivider, S: View>: View {
    ...
    public init(@ViewBuilder top: @escaping () -> P, @ViewBuilder bottom: @escaping () -> S) where D == Splitter {
        ...
        self.init(
            fraction: fraction, hide: hide, constraints: constraints, onDrag: nil, primary: { top() },
            splitter: { D() }, secondary: { bottom() })
    }

    private init(
        fraction: FractionHolder, hide: SideHolder, constraints: SplitConstraints, onDrag: ((CGFloat) -> Void)?,
        @ViewBuilder primary: @escaping () -> P, @ViewBuilder splitter: @escaping () -> D,
    ...
    public func splitter<T>(@ViewBuilder _ splitter: @escaping () -> T) -> VSplit<P, T, S> where T: View {
        return VSplit<P, T, S>(
            fraction: fraction, hide: hide, constraints: constraints, onDrag: onDrag, primary: { primary },
    ...
    public func onDrag(_ callback: ((CGFloat) -> Void)?) -> VSplit {
        return VSplit(
            fraction: fraction, hide: hide, constraints: constraints, onDrag: callback, primary: { primary },
            splitter: { splitter }, secondary: { secondary })
    }
    ...
    public func hide(_ side: SideHolder) -> VSplit<P, D, S> {
        VSplit(
            fraction: fraction, hide: side, constraints: constraints, onDrag: onDrag, primary: { primary },
            splitter: { splitter }, secondary: { secondary })
    }
