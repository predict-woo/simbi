...
private struct TimelineDrawer {
    ...
    private func xOfF(_ frame: Int) -> CGFloat {
        xOfT(Double(frame) * Inspector.frameSeconds)
    }
    ...
    private func drawFrameLanes() {
        ...
        let firstVisible = max(
            ...
            Int(max(0, windowStart) / Inspector.frameSeconds))
        for frame in firstVisible..<(state.firstBufferedFrame + state.frames.count) {
            ...
            context.fill(
                ...
                with: .color(Inspector.speakerColor(record.dominantSlot)))
        }
    }
    ...
    private func drawBufferBand() {
        ...
        context.fill(
            ...
            with: .color(Color.statusWarning.opacity(0.13)))
        context.fill(
    ...
    private func drawSegments() {
        for segment in state.segments {
            ...
            let path = Path(roundedRect: rect, cornerRadius: Design.Radius.badge)
            var isDone = false
    ...
    private func drawDiscards() {
        for span in state.discards {
            ...
            let label = String(
                format: "%.1f s trimmed",
                Double(span.end - span.start) * Inspector.frameSeconds)
    ...
    private func drawBadges() {
        ...
        where liveT - Double(mark.frame) * Inspector.frameSeconds < 14 {
            badges.append((xOfF(mark.frame), "✂ " + InspectorTimelineState.tag(mark.rule)))
        }
        for segment in state.segments.suffix(12)
        where liveT - Double(segment.endFrame) * Inspector.frameSeconds < 14 {
            badges.append(
        ...
        for (index, badge) in badges.enumerated() where badge.x > padL {
            ...
            context.stroke(path, with: .color(Color.hairline), lineWidth: 1)
    ...
    private mutating func drawPointers() {
        pointer(
            x: xOfT(liveT), color: .statusLive,
            label: "live " + Design.time(latest.sessionBaseSeconds + liveT), preferredRow: 0)
        pointer(
            x: xOfF(latest.flushedUpTo), color: Color.statusWarning,
    ...
    private func drawLeftFade() {
        context.fill(
            ...
            with: .linearGradient(
                Gradient(colors: [Color.well, Color.well.opacity(0)]),
